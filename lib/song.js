...
async function getEliteProTech(youtubeUrl) {
  ...
  if (res?.data?.success && res?.data?.downloadURL) return { download: res.data.downloadURL, title: res.data.title, views: res.data.views, likes: res.data.likes, duration: res.data.duration };
...
async function getYupra(youtubeUrl) {
  ...
  const res = await tryRequest(() => axios.get(apiUrl, AXIOS_DEFAULTS));
  if (res?.data?.success && res?.data?.data?.download_url) return { download: res.data.data.download_url, title: res.data.data.title, views: res.data.data.views, likes: res.data.data.likes, duration: res.data.data.duration };
...
async function getOkatsu(youtubeUrl) {
  ...
  const res = await tryRequest(() => axios.get(apiUrl, AXIOS_DEFAULTS));
  if (res?.data?.dl) return { download: res.data.dl, title: res.data.title, views: res.data.views, likes: res.data.likes, duration: res.data.duration };
...
async function searchYoutube(query) {
  ...
  return {
    ...
    seconds: v.seconds || v.duration?.seconds || 0,
    views: Number.isFinite(Number(v.views)) ? Number(v.views) : null,
    likes: Number.isFinite(Number(v.likes)) ? Number(v.likes) : null,
...
async function fetchSong(query) {
  ...
  return {
    ...
    views: result.views ?? video.views ?? null,
    likes: result.likes ?? video.likes ?? null,
    ext: 'mp3',
    mimetype: 'audio/mpeg',
