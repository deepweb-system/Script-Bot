...
async function delayBeforeNextSend(ms = 450) {
  await new Promise(resolve => setTimeout(resolve, ms));
}
async function reactToCommand(sock, msg, emoji = settings.commandReaction) {
  if (!emoji || !msg?.key?.remoteJid || !msg?.key?.id) return;
  try {
    await delayBeforeNextSend();
    await sock.sendMessage(msg.key.remoteJid, { react: { text: emoji, key: msg.key } });
  } catch (e) { console.error('[cmd] reaction failed:', e.message); }
}
...
async function handleSong(sock, msg, text) {
  ...
  try {
    // Resolve the video on YouTube first so the status message can show the
    // real matched title instead of echoing the raw search text back.
    const video = await searchYoutube(query);
    if (!video) {
      await sock.sendMessage(owner, { text: withFooter(`❌ No results found for "${query}".`) });
      return true;
    }

    // Always show the search status in the owner's DM, regardless of where
    // the owner issued the command.
    searchingMessage = await sock.sendMessage(owner, {
      text: withFooter(`🔍 Searching ${video.title}`)
    });
    ...
    await editOwnerSongStatus(sock, owner, searchingMessage,
      withFooter(`🎧 Found: ${result.title || video.title}`));
...
async function handleCommand(sock, msg, text) {
  ...
  if (isClear) {
    ...
    const credsLine = s.credsPreserved
      ? '✅ creds.json preserved — linked device stays connected.'
      : '⚠️ creds.json was not found in session/ — check your session folder.';
    await sock.sendMessage(owner, {
      text: `✅ Temp files & session cache cleared!\n\n`
        + `📊 Temp Media:\n`
        + `• Files removed: ${result.tmp.total}\n\n`
        + `🔐 Session Cache (creds.json kept):\n`
        + `• App state sync files: ${s.appState}\n`
        + `• Pre-key files: ${s.preKeys}\n`
        + `• Sender-key files: ${s.senderKeys}\n`
        + `• Session files: ${s.sessions}\n`
        + `• Other files: ${s.other}\n`
        + `• Total session files removed: ${s.removed}\n\n`
        + `${credsLine}\n\n`
        + `© Powered by *Muaz*.`
    });
...
module.exports = { handleIncoming, handleStatusEvent, handleMessageUpdate, cacheContactNames };
