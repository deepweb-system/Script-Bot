...
const { fetchSong } = require('./lib/song');
...
async function handleSong(sock, msg, text) {
  ...
  try {
    console.log(`[song] searching/downloading: ${query}`);
    const result = await fetchSong(query);

    if (!result?.buffer?.length) throw new Error('No audio data returned');

    await sock.sendMessage(owner, {
      audio: result.buffer,
      mimetype: result.mimetype,
      fileName: `${String(result.title || 'song').replace(/[\\/:*?"<>|]/g, '_').slice(0, 120)}.${result.ext}`,
      ptt: false
    });

    await delayBeforeNextSend();
    await reactToCommand(sock, msg, settings.songSuccessReaction);
    console.log(`[song] sent "${result.title}" via ${result.source}`);
  } catch (e) {
    console.error('[song] download failed:', e?.stack || e?.message || e);
    await sock.sendMessage(owner, {
      text: withFooter(`❌ Failed to download the song.\n\n${e?.message || 'All download sources failed.'}`)
    });
    await delayBeforeNextSend();
    await reactToCommand(sock, msg, settings.songFailureReaction);
  }
