...
The `/clear` command removes temporary cached media, clears the in-memory message/status stores, **and** cleans out the WhatsApp `session/` folder down to the one file that actually matters.
...
`/clear` deletes **every file inside `session/` except `creds.json`**.
...
### 🛡️ 11. Anti-Delete / Anti-Edit Per-Chat Toggles

On top of the global defaults in `settings.js`, anti-delete and anti-edit can each be switched on or off **per chat** (per group, or per individual person):
...
| `/a-edit` | None | `/a-edit on` / `/a-edit off` | Per-chat anti-edit toggle |
| `/reset-antiedit` | None | Send in DM only | Resets all anti-edit overrides |
